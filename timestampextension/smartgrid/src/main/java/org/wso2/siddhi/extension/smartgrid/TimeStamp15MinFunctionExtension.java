...
public class TimeStamp15MinFunctionExtension extends FunctionExecutor {
    private static final Logger log = Logger.getLogger(TimeStamp15MinFunctionExtension.class);
    private Attribute.Type returnType = Attribute.Type.LONG;

    @Override
    protected void init(ExpressionExecutor[] attributeExpressionExecutors, ExecutionPlanContext executionPlanContext) {
        if (attributeExpressionExecutors.length != 1) {
            throw new ExecutionPlanValidationException("Invalid no of arguments passed to smartgrid:timeStamp15min() function, " +
