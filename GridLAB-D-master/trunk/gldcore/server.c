...
SOCKET sd;
bool wso2_socket_not_initialized = true;
...
void *http_response(void *ptr)
{
	if(wso2_socket_not_initialized){
		wso2_socket_not_initialized = false;
	//Initialize WSO2 server's client socket
	//Connect with the local server which is listening on port 7777
	...
	memset(&serv_addr2,0,sizeof(serv_addr));
	serv_addr2.sin_family = AF_INET;
	memcpy((char*)&serv_addr2.sin_addr.s_addr,(char*)server->h_addr,server->h_length);
	serv_addr2.sin_port = htons(7777);

Retry2:
	/* open connection */
	if ( connect(sd,(struct sockaddr*)&serv_addr2,sizeof(serv_addr2)) < 0 )
	{
		printf("A-->Unable to connect to server.\r\n");
		sleep(1);
		goto Retry2;
	}
	}
...
//		const char* serviceHead="curl -X GET ";
//		const char* serviceTail = " -H \"Content-Type: text/plain\" http://10.100.4.160:6267/";
//
...
//			printf("--->%d\r\n", pch);
			...
			if(*pch == delimiter){
				...
				//printf("------->%d\r\n", iPtr - iPtr2);
				//printf("=======>%s\r\n", pch2+iPtr2);
				strncpy(line, pch2+iPtr2, iPtr - iPtr2);
				//strncpy(line, "abcdefg", 5);
				line[iPtr - iPtr2] = 0;
				//printf("%.*s\r\n", iPtr - iPtr2, pch2 + iPtr2);
				char* item = trimwhitespace(line);
				//printf("-->%s<---\r\n", item);
