...
static int write_multi_recorder(struct recorder *my, char *ts, char *value, char *receiver, char *id)
{
    ...
    char *re ="auction";
    char *rec ="controller";
    //aution uses regex ----------------- change to xml as well
    ...
    //uses xml
        ...
        int i = 0;
            ...
            strcat(data,ts);
            strcat(data,"</time><iij_timestamp>56783</iij_timestamp></payloadData></event></events>");
...
static TIMESTAMP multi_recorder_write(OBJECT *obj)
{
	char256 receiver;
	char256 id;
	if(obj->parent->parent!=NULL){//house name
		sprintf(id, "%s", obj->parent->parent->name);
	}else{//market name
		sprintf(id, "%s", obj->parent->name);
	}
	strcpy(receiver,obj->parent->oclass->name);
	struct recorder *my = OBJECTDATA(obj,struct recorder);
	char ts[64]="0"; /* 0 = INIT */
	if (my->format==0)
	{
	...
	if ((my->limit>0 && my->samples > my->limit) /* limit reached */
		|| write_multi_recorder(my, ts, my->last.value, receiver, id)==0) /* write failed */
	{
